...
To test for ApoE4 dose effects on each metabolite, $p=230$ model comparisons were needed, hence the function \texttt{nested} (see Appendix \ref{app:nested}) was created to iterate over the metabolites, fit the nested models, compare them using ANCOVA F-tests, store and adjust the p-values, filter those below .05, then consolidate the coefficients of the meaningful full models and the p-values of their t-tests in a table and display it. To decrease run time, as well as harness the power of multiple cores, \textsf{furrr}'s \texttt{future\_map} function was used for parallel iterations.

The dependent variable in each model was a metabolite; the nested model \eqref{rm}, has only clinical variables, while the full model \eqref{fm}, features the clinical variables, plus the number of ApoE4 alleles (0, 1 or 2 -nominal) as explanatory variables. Let $y_j$ represent the $j$-th metabolite, $x_k$ the $k$-th clinical variable, and $D_{\varepsilon4}$ the ApoE4 dose; the nested models then are:
\begin{align}
...
\begin{algorithm}
\caption{Benjamini–Hochberg Procedure to Control the FDR}\label{alg:fdr}
...
\leavevmode\newline \textbf{Implementation and Evaluation}\hspace{.25cm} Several models were fitted, thus two functions were created to facilitate the process: \texttt{multifit} (see Appendix \ref{app:multifit}) to fit the models and \texttt{evaluate} (see Appendix \ref{app:evaluate}) to display the performance metrics and plot the ROC curves.
...
\appendix 
\section{R functions}
\subsection{nested} \label{app:nested}
...
\subsection{multifit} \label{app:multifit}
\begin{verbatim}
multifit <- function(X,
                     y,
                     model,
...
\subsection{evaluate} \label{app:evaluate}
