...
The data were processed using base R and \textsf{dplyr} \cite{dplyr}. The package \textsf{furrr} was used to enable multi-core processing whenever applicable.
...
In machine learning, a class denotes a group of objects that share common characteristics, such as having AD or ApoE4 \cite*{Drummond2010}. Classification, in this context, denotes training a (supervised) learning algorithm on labeled data (containing their class). The classifier learns patterns in the data and is then able to assign a class to unlabeled data.

In this study, the metabolites on top of background clinical characteristics where fitted in several models to predict having AD and/or at least one ApoE4. First, a benchmark model was fitted with the clinical characteristics only. Then the full metabolomics panel was fitted on top of the clinical characteristics
