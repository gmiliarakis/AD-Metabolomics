...
\subsection{ApoE4 dose effects on serum metabolite levels}
...
The nested linear model comparison did not reveal any significant ApoE4 dose effects, after correcting FDR. This can be potentially explained considering the covariates in each approach. The global test was corrected for sex only, while the nested models fitted several background clinical factors. That is, the metabolic variance explained by ApoE4 dose in the global test might be partially attributed to other clinical factors, such as hypercholesterolemia (that often coexists with hypertriglyceremia and is not measured in this study). Notably, the metabolites showing trends of an Apoe4 dose effect were different between the AD and SCD group. Consistent with the global test, AD group presented mainly a lipid (TG \& DG) signature, while the SCD group an amino one (serine, tryptophan, glycine, homoserine and putrescine).
