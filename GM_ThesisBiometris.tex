...
\multirow{4}{*}{ApoE4 alleles}           & 1x                         & 31 & 61  & 124   \\
                                 & 2x                         & 3  & 36  & 29    \\
                                 & $\geq$ 1x                & 34 & 87  & 121   \\
...
The data were processed using base R functions and \textsf{dplyr} \cite{dplyr}. The package \textsf{furrr} was used to enable multi-core processing whenever applicable.
...
\clearpage
\section{Results}
\subsection{RQi - Differential levels of serum metabolites per ApoE4 dose among AD and SCD}
...
Several metabolites from all classes were affected by ApoE4 dose, both in AD and SCD. Among AD patients, ApoE4 dose seems to have an effect on several triglycerides, diglycerides, L-glutamic acid, L-lysine, 2- and 3-hydroxybutyric acid, glutamic acid, 3-phosphoglyceric acid, phosphatidylcholin, sphyngomyelin, HpH.PAF.C17, phosphoethanolamine, imino-diacetate (Table \ref{tab:nestedSCD}). Among individuals with SCD, lipid metabolites were not affected as much as in the AD group, with only LPC and PC showing a difference. Aminoacids (glycine, taurine, L-serine, serotonine, L-glutamic acid, L-glutamic acid, phosphoethanolamine) and oxidative stress compounds were mostly affected in this group (Table \ref{tab:nestedSCD}). However, after correcting for multiple testing with FDR, none of the p-values were significant at $\alpha=0.05$.
\begin{table}[H]
\caption{ApoE4 dose effects on metabolites: significant F tests ($\alpha=0.05$) from nested linear model comparison in AD. Reduced model: clinical data only, Full model: clinical data + ApoE4 dose}
\label{tab:nestedAD}
\begin{tabular}{llll} \toprule
                          & Effect & P(\textgreater{}F) & FDR-adjusted \\ \midrule
Am.L.Glutamic.acid        & +      & 0.011              & 0.314        \\
...
OS.HpH.PAF.C16.0          & +      & 0.038              & 0.345       \\ \bottomrule
\end{tabular}
\end{table}

\begin{table}[]
\caption{ApoE4 dose effects on metabolites: significant F tests ($\alpha=0.05$) from nested linear model comparison in SCD. Reduced model: clinical data only, Full model: clinical data + ApoE4 dose}
\label{tab:nestedSCD}
\begin{tabular}{lllll} \toprule
                         & 1x ApoE4 & 2x ApoE4 & P(\textgreater{}F) & FDR-adjusted \\ \midrule
Am.Ethanolamine          & +        & -        & 0.014              & 0.444        \\
...
\begin{tabular}{lllll} \toprule
                                     & NIR & Accuracy (95/\% CI)     & P(NIR \textless Accuracy) & AUC   \\ \midrule
Clinical features only & \multirow{5}{*}{0.3522} & 0.4807 (0.4744, 0.4869) & \textless 2.2e-16 & 0.814 \\
Clinical features + 230 metabolites  &     & 0.5751 (0.5689, 0.5813) & \textless 2.2e-16         & 0.834 \\
Clinical features + 6 latent factors &     & 0.5313 (0.525, 0.5375)  & \textless 2.2e-16         & 0.818 \\
Decision Tree                        &     & 0.5082 (0.502, 0.5145)  & \textless 2.2e-16         & 0.818 \\
XGBoost                              &     & 0.5944 (0.5882, 0.6005) & \textless 2.2e-16         & 0.836 \\ \bottomrule
\end{tabular}
\end{table}
