...
load("alldata.Rdata")
library(globaltest)
mets <- ESetMbolCSFPR2@assayData$exprs
y <- as.numeric(y) -1
mets <- t(mets)
gt <- globaltest::gt(APOE ~ .-sex-Diagnosis, data = df, model = "multinomial")
...
#  Logistic Regression
lr <- fit(title = 'Logistic Regression',
  X = scale(ADmets),
  y = apoe,
  model = "glm",
  ctrl = ctrl
...
tree <- fit(
  ...
  model = "rpart2",
    ctrl = ctrl,
  tuneGrid = expand.grid(maxdepth=c(2,3,4))
)
