...
fit <- function(title,
  ...
  cm <- confusionMatrix(mdl$pred$obs, mdl$pred$pred, positive = "E4YES")
...
mlogit <- train(
  ...
  trControl = trainControl(
                  method = "cv", number = 10,
                  classProbs = TRUE, summaryFunction = multiClassSummary, savePredictions = TRUE, sampling = 'smote'
                ),
...
obs <- as.numeric(obs)
yhats <- as.numeric(preds)
roc <- multiclass.roc(obs,yhats)
metrics <- cbind(lr$metrics, tree$metrics, rf$metrics)
rocs <- list(tree$roc, rf$roc)
