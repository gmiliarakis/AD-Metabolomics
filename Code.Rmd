...
Performing a Global Test on the serum metabolites of AD patients, correcting for sex (Ho: E4 status has no effect on metabolite levels, Ha: it has an effect), yields a significant difference (p = 0.029 ) between E4 carriers and non-carriers. The presence of ApoE4, correcting for sex seems to have an effect on the metabolites shown in the plot below. The most significantly affected metabolites are triglycerides, diglycerides (positively) and γ-aminobutyric acid (negatively).
...
ggplot2::ggplot(fimp, aes(x = reorder(rownames(fimp), Overall), y = Overall, fill = reorder(rownames(fimp), Overall))) +
  theme_tufte() +
  geom_point(size = 4, alpha = 0.6, aes(x= rownames(fimp), y = Overall, fill= rownames(fimp))) +
...
ggplot(fimp, aes(x = rownames(fimp$importance), y = Overall, fill = rownames(fimp$importance)), top = 20) +
