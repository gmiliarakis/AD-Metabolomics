...
cm.lr <- confusionMatrix(labels,yhat, positive='1')
...
tree <- train(y ~.,
             train,
             method = "ada",
             tuneGrid=expand.grid(iter=10, maxdepth=5, nu=0.01),
             metric = "ROC",
             trControl = ctrl)
#Predict
...
metrics.rf <- rbind(metrics.rf, data.frame(AUC=perf.rf@y.values[2]))
