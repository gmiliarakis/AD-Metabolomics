...
fit <- function(title,
  ...
  for (i in 1:times) {
    ...
    ys <- c(ys, labels)
    # Train the model
    mdl <- caret::train(train[, 1:ncol(train)], train$y,
      method = model,
      tuneGrid = grid,
      trControl = ctrl,
      metric = 'ROC',...
    ...
    cm <- caret::confusionMatrix(labels, predict(mdl, test), positive = "X1")
...
rf <- fit(
  ...
  ctrl = trainControl(
                ...
                grid = expand.grid(mtry = 10, splitrule = 'gini', min.node.size=10),
  times = k,
  num.trees = 50
)
